...
function Invoke-Check {
  ...
  try { Dc -f $ComposeFile config | Out-Null; Write-Log "docker-compose config OK" 'SUCCESS' } catch { Write-Log "docker-compose config failed: $($_.Exception.Message)" 'ERROR' }
...
function Invoke-ComposeDownFresh { if (Test-Path $ComposeFile) { Dc -f $ComposeFile --env-file $EnvFile down -v | Out-Null } }
function Invoke-BuildAll {
  Write-Log "Building images (docker compose build)"
  try { Dc --env-file $EnvFile -f $ComposeFile -f $OverrideFile build | Out-Null; Write-Log "Build completed" 'SUCCESS' } catch { Write-Log ("Build failed: {0}" -f $_.Exception.Message) 'ERROR'; throw }
}
...
function Invoke-PhaseCore {
  ...
  Dc --env-file $EnvFile -f $ComposeFile -f $OverrideFile up -d $DockerUpArgs postgres neo4j rabbitmq
...
function Invoke-PhaseMonitoring {
  ...
  try { Dc --env-file $EnvFile -f $ComposeFile -f $OverrideFile up -d $DockerUpArgs prometheus grafana loki promtail alertmanager otel-collector } catch { Write-Log $_.Exception.Message 'WARN' }
...
function Invoke-PhaseOrchestration {
  ...
  Dc --env-file $EnvFile -f $ComposeFile -f $OverrideFile up -d $DockerUpArgs n8n
...
function Invoke-PhaseAPI {
  ...
  Dc --env-file $EnvFile -f $ComposeFile -f $OverrideFile build api-service
  Dc --env-file $EnvFile -f $ComposeFile -f $OverrideFile up -d $DockerUpArgs api-service
  Wait-Healthy 'enhanced-ai-agent-api' 300
}

function Write-Summary {
  Write-Host "`n=== docker compose ps ==="
  Dc -f $ComposeFile -f $OverrideFile ps
...
if (-not $SkipPull) { Write-Log "docker compose pull"; try { Dc -f $ComposeFile -f $OverrideFile --env-file $EnvFile pull | Out-Null } catch { Write-Log $_.Exception.Message 'WARN' } }
